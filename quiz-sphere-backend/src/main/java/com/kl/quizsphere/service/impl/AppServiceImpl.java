...
import com.kl.quizsphere.mapper.AppMapper;
import com.kl.quizsphere.model.dto.app.AppQueryRequest;
...
import com.kl.quizsphere.model.vo.AppVO;
...
public class AppServiceImpl extends ServiceImpl<AppMapper, App> implements AppService {
    ...
    public void validApp(App app, boolean add) {
        ...
        if (add) {
            ...
            AppTypeEnum appTypeEnum = AppTypeEnum.getEnumsByValue(appType);
        ...
        if (StringUtils.isNotBlank(appName)) {
            ThrowUtils.throwIf(appName.length() > 128, ErrorCode.PARAMS_ERROR, "应用名称不能超过128个字符");
        }
        if (StringUtils.isNotBlank(appDesc)) {
            ThrowUtils.throwIf(appDesc.length() > 2048, ErrorCode.PARAMS_ERROR, "应用描述不能超过2048个字符");
        }
        if (reviewStatus!=null) {
            ReviewStatusEnum reviewStatusEnum = ReviewStatusEnum.getEnumsByValue(reviewStatus);
    ...
    public QueryWrapper<App> getQueryWrapper(AppQueryRequest appQueryRequest) {
        ...
        queryWrapper.like(StringUtils.isNotBlank(reviewMessage), "appIcon", reviewMessage);
        // 精确查询
